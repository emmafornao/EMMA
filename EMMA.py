...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.installed_model.setHorizontalHeaderLabels(
            [
                "name",
                "mod id",
                "dateUpdated",
        ...
        self.favourites_model.setHorizontalHeaderLabels(
            [
                "name",
                "mod id",
                "dateUpdated",
                ...
                if not os.path.exists(self.config_file_path):
                    if "config" in str(self.config_file_path):  # Check if it's the favourites file
                        with open(self.config_file_path, "w") as file:
                            self.config = {"game_path": "", "library_path": "", "mods_path": ""}
    ...
    def load_data_from_json(self, file_path, model):
        try:
            ...
            for mod in installed_mods:
                ...
                row = [
                    QStandardItem(name),
                    QStandardItem(str(modId)),
                    QStandardItem(dateUpdated),
                    QStandardItem(dateInstalled),
                    QStandardItem(pathOnDisk),
                    QStandardItem(installedVersion),
                    QStandardItem(str(installedVersionID)),
                    QStandardItem(latestVersion),
                    QStandardItem(str(latestVersionID)),
                ]
            ...
            favourites_data["installedMods"].append(
                {
                    ...
                    "latestUpdatedFile": {"filename": latest_version, "iD": latest_version_id},
                }
            )
    ...
    def uninstall_mods(self, mod_ids_to_remove, keep_library_entry=False):
        ...
        if self.is_process_running("ArkAscended.exe"):
            ...
            try:
                with (
                    open(self.config.get("library_path", ""), "r", encoding="utf-8-sig") as file
                ):  # loading from file instead of self.library just in case something changed. could also refresh self.library right before instead
                    data = json.load(file)  # Load the JSON data
                ...
                if dialog.exec() == QtWidgets.QDialog.DialogCode.Accepted:
                    ...
                    if not keep_library_entry:
                        ...
                        updated_installed_mods = [
                            mod
                            for mod in installed_mods
                            if mod.get("details", {}).get("iD", "") not in mod_ids_to_remove
                        ]
    ...
    def update_library_entries(
        self, mod_ids, mod_entries
    ):  # meant to update one or more at once. avoid calling it over and over again
    ...
    def reinstall_mods(self, mod_ids):
        ...
        for mod in mod_ids:
            ...
            path_on_disk = next(
                (
                    item["pathOnDisk"]
                    for item in self.library["installedMods"]
                    if item.get("details", {}).get("iD") == mod
                ),
                None,
            )
